...
namespace Game.Rendering.GPUSkinning.Editor
{
    public class AnimationFixer : EditorWindow
    {
        ...
        [MenuItem("Tools/GPUSkinning/修复动画Path Missing (AnimationFixer)")]
        ...
        private void OnGUI()
        {
            ...
            if (m_Animator == null)
            {
                EditorGUILayout.HelpBox("请设置animator", MessageType.Info);
                return;
            }
            ...
            using (var scrollview = new EditorGUILayout.ScrollViewScope(m_ScrollPosition))
            {
                ...
                {
                    EditorGUILayout.HelpBox("没有 Missing", MessageType.Info);
                    return;
                }

                using (new EditorGUILayout.HorizontalScope())
                {
                    GUILayout.Label("■ Missing 路径");
                    GUILayout.Space(10);
                    GUILayout.Label("■ 置換后 路径");
                }
                ...
                GUILayout.Label("■ 修正対象的AnimationClip");
